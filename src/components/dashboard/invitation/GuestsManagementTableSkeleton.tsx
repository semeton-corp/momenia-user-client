...
// Skeleton yang meniru struktur GuestsManagementTable (judul, toolbar, tabel,
// footer) selagi GET .../guest-invitations masih loading.
export function GuestsManagementTableSkeleton({ rows = 6 }: { rows?: number }) {
  return (
    <WorkspaceCard className="flex flex-col gap-4 px-4 py-4 sm:px-6 xl:gap-0 xl:py-8 xl:px-5 border-0 shadow-none xl:border xl:shadow-sm xl:min-h-[798px]">
      <div className="h-7 w-40 animate-pulse rounded bg-zinc-100 xl:mb-8" />

      {/* Toolbar */}
      <div className="flex items-center gap-2 xl:mb-4 xl:justify-between">
        <div className="h-9 min-w-0 flex-1 animate-pulse rounded-lg bg-zinc-100 xl:w-[402px] xl:flex-none" />
        <div className="flex shrink-0 items-center gap-2">
          <div className="hidden h-7 w-28 animate-pulse rounded-lg bg-zinc-100 xl:block" />
          ...
          <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
            ...
            <tbody>
              {Array.from({ length: rows }).map((_, i) => (
                <tr key={i}>
                  <td className="border-b border-zinc-100 px-3 py-3"><div className="h-4 w-4 animate-pulse rounded bg-zinc-100" /></td>
                  <td className="border-b border-zinc-100 px-3 py-3"><div className="h-5 w-5 animate-pulse rounded-md bg-zinc-100" /></td>
                  ...
                  <td className="border-b border-zinc-100 px-4 py-3"><div className="h-8 w-24 animate-pulse rounded-lg bg-zinc-100" /></td>
